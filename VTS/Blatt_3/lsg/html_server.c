...
void html_serv(int sockfd, int rootfd) {
	...
	memset((void*)url, '\0', MAXREQUESTSIZE);
	...
	sscanf(in, "%8s /%255s HTTP/%s", method, url, version);
...
void response_get(int sockfd, int rootfd, const char* url) {
	int filefd = openat(rootfd, url, O_RDONLY, "rb");
	if (filefd == -1) {
		printf("Not found\n");
...
void response_post(int sockfd, const char* in) {
	...
	sprintf(responseContent, "<!DOCTYP HTML><HTML><BODY><center><h1>Ergebnis: %d </h1></center></BODY></HTML>", ergebnis);

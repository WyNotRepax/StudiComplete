...
void Model::loadMeshes(const aiScene* pScene, bool FitSize)
{
	...
	for (unsigned int meshIndex = 0; meshIndex < pScene->mNumMeshes; meshIndex++) {
		...
		for (unsigned int vertexIndex = 0; vertexIndex < pAiMesh->mNumVertices; vertexIndex++) {
			...
			aiVector3D* pTexture = pAiMesh->mTextureCoords[0] + vertexIndex;
			aiVector3D* pTangent = pAiMesh->mTangents + vertexIndex;
			aiVector3D* pBiTangent = pAiMesh->mBitangents + vertexIndex;
			...
			if (pAiMesh->mTextureCoords[0] != nullptr) {
				pMesh->VB.addTexcoord0(pTexture->x, 1-pTexture->y, pTexture->z);
			}
			//pMesh->VB.addTexcoord1(pTangent->x, pTangent->y, pTangent->z);
			//pMesh->VB.addTexcoord2(pBiTangent->x, pBiTangent->y, pBiTangent->z);
...
void Model::loadMaterials(const aiScene* pScene)
{
	...
	for (unsigned int materialIndex = 0; materialIndex < pScene->mNumMaterials; materialIndex++) {
		...
		aiString normalAiString;
		std::string normalPath = Path;
		if (aiGetMaterialString(pAiMaterial, AI_MATKEY_TEXTURE_DIFFUSE(0), &textureAiString) != aiReturn_SUCCESS) {
			printf("Could not get Texture\n");
		}
		else {
			normalPath.append(textureAiString.C_Str());
		}
		size_t ext_pos = normalPath.find_last_of('.');
		normalPath.insert(ext_pos, "_n");
		pMaterial->NormalTex = Texture::LoadShared(normalPath.c_str());

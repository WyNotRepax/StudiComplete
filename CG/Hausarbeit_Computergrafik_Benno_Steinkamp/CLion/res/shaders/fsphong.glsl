...
void main()
{
    ...
    vec3 Color = (AmbientColor)*DiffTex.rgb;
    for (int i=0; i<LightCount;i++){
        ...
        vec3 LightColor = (DiffuseComponent * DiffTex.rgb + SpecularComponent) * I;

        if (lights[i].Type == 0|| lights[i].ShadowIndex == -1){
            // Point Light
            // Or Light Without Shadows
            Color += LightColor;
        } else {
            vec4 PosSM = ShadowMapMat[lights[i].ShadowIndex] * vec4(Position, 1);

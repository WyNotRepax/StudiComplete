...
public:
    ...
    void attenuation(const float a, const float b, const float c) { attenuation(Vector(a, b, c)); };
...
public:
    DirectionalLight(const Vector &d = Vector(-1, -1, -1), const Color &c = ::Color(1, 1, 1)) : BaseLight(
            Vector(10, 10, 10), c), Direction(d) {}

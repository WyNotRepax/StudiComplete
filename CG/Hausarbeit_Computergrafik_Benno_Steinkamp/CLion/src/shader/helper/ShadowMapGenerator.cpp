...
Matrix ShadowMapGenerator::calcProjection(BaseLight *pLight, const AABB &BBox, const Matrix &View) const {
    ...
    if (pLight->type() == BaseLight::SPOT) {
        ...
        Proj.orthographic(MaxU * 2 + 0.1f, MaxV * 2 + 0.1f, 0.001f, 2*BBox.size().length() + 5.0f);
    }
...
void ShadowMapGenerator::generate(std::list<BaseModel *> &Models) {
    ...
    //DebugRender::ref().drawBoundingBox(SceneBoundingBox);
    //printf("Bounding box: %f,%f,%f %f,%f,%f\n", SceneBoundingBox.Min.X, SceneBoundingBox.Min.Y, SceneBoundingBox.Min.Z,
     //      SceneBoundingBox.Max.X, SceneBoundingBox.Max.Y, SceneBoundingBox.Max.Z);

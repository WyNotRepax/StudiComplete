...
#include "helper/ShaderLightMapper.h"

const BaseShader* BaseShader::ShaderInPipe = nullptr;
...
char* BaseShader::loadFile( const char* File, unsigned int& Filesize )
{
    ...
    if(!pFile)
    {
        ...
        printf( "Unable to open file %s errno: %d", File,errno);
        return nullptr;
    }

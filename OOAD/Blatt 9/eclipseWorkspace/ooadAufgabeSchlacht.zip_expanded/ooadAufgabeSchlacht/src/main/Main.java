...
public class Main {
	...
	private Ruestung[] ruestungen = { new Ruestung(Messages.getString("Main.0"), 4, 20), //$NON-NLS-1$
	...
	public void neueRuestung() {
		...
		Kaempfer k = this.kaempfer.get(this.auswahl(this.kaempfer.toArray()));
		k.nimmRuestung(r);
	}
	...
	public void neuerSoeldner() {
		...
		default: {
			throw new IllegalArgumentException(Messages.getString("Main.Exception") + ": " + wahl);
		}
		}
		this.kaempfer.add(k);
	...
	private void kaempfen() {
		while (true) {
			for (Kaempfer k : this.kaempfer) {
				...
				System.out.printf("Gesundheit von %s: %d\n", k, k.getGesundheit());
				System.out.printf("Gesundheit von %s: %d\n", this.sr, this.sr.getGesundheit());
				if (this.sr.getGesundheit() <= 0) {
					break;
				}
			}
			this.kaempfer.removeIf(k -> k.getGesundheit() <= 0);
			if (sr.getGesundheit() <= 0) {
				System.out.println("Du hast gewonnen!");
				break;
			}
			if (this.kaempfer.size() <= 0) {
